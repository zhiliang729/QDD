...
class UserService {
    ...
    class func user(id: Int, success: ((User) -> Void)?, fail: ((RequestError) -> Void)? ) {
        HttpRequst.get(Daker.userInfo(id: id).routeUrl, parameters: nil, onSuccess: { (res, json) in
    ...
    //MARK: - 2. 获取用户评论
    class func comments(userID: Int, start: Int, success: ((UserCommentsHandler) -> Void)?, fail: ((RequestError) -> Void)? ) {
