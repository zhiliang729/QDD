...
extension ShareSDK {
    ...
    /// 成功之后同步用户结果
    typealias UserSyncHandler = (user: SSDKUser, handler: ResultHandler) -> Void
     ...
     */
    class func login(type: ThirdLoginType, userSync: UserSyncHandler, result: ResultHandler?) {
        ...
        SSDKAuthViewStyle.setCancelButtonLabelColor(UIColor(hex: 0x303030))
        ...
        ShareSDK.getUserInfo(shareSDKType) { (state, user, error) in
            if (state == .Success) { //成功
                userSync(user: user) { tmpResult in
                    result?(tmpResult)
                }
...
extension ShareSDK {
    ...
    class func share(type: SSDKPlatformType, content: String, imgURL: String, title: String, desc: String, contentURL: String) {
        ...
        guard let shareUrl = NSURL(string:contentURL) else {
        ...
        SSUIEditorViewStyle.setCancelButtonLabelColor(UIColor(hex: 0x303030))
        
        SSUIEditorViewStyle.setShareButtonLabel("发布")
        SSUIEditorViewStyle.setShareButtonLabelColor(UIColor(hex: 0x303030))
        ...
        if type == .SubTypeWechatSession || type == .SubTypeWechatTimeline {
            ...
            params.SSDKSetupShareParamsByText(content, images: imgURL, url: shareUrl, title: title, type: .Auto)
            ...
            params.SSDKSetupSinaWeiboShareParamsByText(content + " " + shareUrl.absoluteString, title: title, image: imgURL, url: shareUrl, latitude: 0, longitude: 0, objectID: nil, type: .Image)
            ...
            params.SSDKSetupQQParamsByText(content, title: title, url: shareUrl, thumbImage: nil, image: imgURL, type: .Auto, forPlatformSubType: .SubTypeQQFriend)
            share(type, parameters: params, onStateChanged: { (state, _, _, _) in
                handler(state)
            })
            ...
            params.SSDKSetupQQParamsByText(content, title: title, url: shareUrl, thumbImage: nil, image: imgURL, type: .WebPage, forPlatformSubType: .SubTypeQZone)
            share(type, parameters: params, onStateChanged: { (state, _, _, _) in
                handler(state)
            })
